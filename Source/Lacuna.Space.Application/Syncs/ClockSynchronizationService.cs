...
public class ClockSynchronizationService
{
    ...
    public async Task<bool> SynchronizeProbeAsync(Probe probe, string accessToken)
    {
        ...
        {
            attempts++;
            _logger.LogDebug("Tentativa de sincronização {Attempt} para sonda {ProbeName}", attempts, probe.Name);
            ...
            try
            {
                ...
                var t1Decoded = Probe.DecodeTimestamp(t1.ToString(), probe.Encoding);
                var t2Decoded = Probe.DecodeTimestamp(t2.ToString(), probe.Encoding);

                // Calcula offset e round trip
                ...
                _logger.LogDebug(
                    "Sonda {ProbeName} - Offset: {Offset} ticks, Round trip: {RoundTrip} ticks, Sincronizada: {IsSynchronized}",
                    probe.Name, timeOffset, roundTrip, probe.IsSynchronized);
            ...
            {
                _logger.LogWarning(ex, "Falha ao sincronizar sonda {ProbeName} na tentativa {Attempt}", probe.Name,
                    attempts);
